...
//
//==============================================================
`ifndef READ_WRITE_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define READ_WRITE_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class read_write_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "read_write_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(read_write_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(read_write_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            axi_pkg::axi_slave_sequence#(64,64,8,3,1) axi_slave_mem_0_seq;
            axi_pkg::axi_slave_sequence#(64,64,8,3,1) axi_slave_mem_1_seq;
            ...
            if (!uvm_config_db#(read_write_reference_model)::get(p_sequencer,"", "refm", refm))
                ...
                begin
                    fork
                        begin //axi slave sequence. loop delays
                            `uvm_create_on(axi_slave_mem_0_seq, p_sequencer.mem_0_sqr);
                            axi_slave_mem_0_seq.misc_if = refm.misc_if;
                            axi_slave_mem_0_seq.ap_done    = refm.ap_done_for_nexttrans   ;
                            axi_slave_mem_0_seq.ap_ready   = refm.ap_ready_for_nexttrans  ;
                            axi_slave_mem_0_seq.finish     = refm.finish ;
                            axi_slave_mem_0_seq.isusr_delay = axi_pkg::NO_DELAY;
                            `uvm_send(axi_slave_mem_0_seq);
                        end
                        begin //axi slave sequence. loop delays
                            `uvm_create_on(axi_slave_mem_1_seq, p_sequencer.mem_1_sqr);
                            axi_slave_mem_1_seq.misc_if = refm.misc_if;
                            axi_slave_mem_1_seq.ap_done    = refm.ap_done_for_nexttrans   ;
                            axi_slave_mem_1_seq.ap_ready   = refm.ap_ready_for_nexttrans  ;
                            axi_slave_mem_1_seq.finish     = refm.finish ;
                            axi_slave_mem_1_seq.isusr_delay = axi_pkg::NO_DELAY;
                            `uvm_send(axi_slave_mem_1_seq);
                        end
                        begin
                            ...
                            for(int i=0; i<1; i++) begin
                                logic[63:0] data64bit_mem_0_r[$];
                                logic[32-1:0] databusbit_mem_0_r[$];
                                logic[63:0] data64bit_mem_1_r[$];
                                logic[32-1:0] databusbit_mem_1_r[$];
                                data64bit_mem_0_r.delete(); databusbit_mem_0_r.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                for(int j=0; j < (64+32-1)/32; j++) begin
                                    data64bit_mem_0_r.push_back( ((refm.mem_blk_pages_mem_0.maxi_bundlevar_offset["mem_0_r"]+refm.mem_blk_pages_mem_0.page_ofst[refm.mem_blk_pages_mem_0.rd_page_idx])>>(j*32)) & (2**32-1) );
                                end
                                foreach(data64bit_mem_0_r[s]) databusbit_mem_0_r[s]=data64bit_mem_0_r[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_mem_0_r, 0, 16, 1);
                                data64bit_mem_1_r.delete(); databusbit_mem_1_r.delete();
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                for(int j=0; j < (64+32-1)/32; j++) begin
                                    data64bit_mem_1_r.push_back( ((refm.mem_blk_pages_mem_1.maxi_bundlevar_offset["mem_1_r"]+refm.mem_blk_pages_mem_1.page_ofst[refm.mem_blk_pages_mem_1.rd_page_idx])>>(j*32)) & (2**32-1) );
                                end
                                foreach(data64bit_mem_1_r[s]) databusbit_mem_1_r[s]=data64bit_mem_1_r[s][32-1:0];
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=1;
                                axi_master_wr_control_seq.datamerge_inavg(databusbit_mem_1_r, 0, 28, 1);
            ...
            p_sequencer.mem_0_sqr.stop_sequences();
            p_sequencer.mem_1_sqr.stop_sequences();
