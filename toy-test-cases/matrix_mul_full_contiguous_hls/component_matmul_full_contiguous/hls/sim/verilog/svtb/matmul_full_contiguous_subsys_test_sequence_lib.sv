...
//
//==============================================================
`ifndef MATMUL_FULL_CONTIGUOUS_SUBSYS_TEST_SEQUENCE_LIB__SV                                              
    `define MATMUL_FULL_CONTIGUOUS_SUBSYS_TEST_SEQUENCE_LIB__SV                                          
                                                                                                    
                                                                                                    
    `include "uvm_macros.svh"                                                                     
                                                                                                    
    class matmul_full_contiguous_subsys_test_sequence_lib extends uvm_sequence;                                
                                                                                                    
        function new (string name = "matmul_full_contiguous_subsys_test_sequence_lib");                      
            super.new(name);                                                                        
            `uvm_info(this.get_full_name(), "new is called", UVM_LOW)                             
        endfunction                                                                                 
                                                                                                    
        `uvm_object_utils(matmul_full_contiguous_subsys_test_sequence_lib)                                     
        `uvm_declare_p_sequencer(matmul_full_contiguous_virtual_sequencer)                                     
        ...
        virtual task body();                                                                        
            ...
            axi_pkg::axi_slave_sequence#(64,64,8,3,1) axi_slave_gmem_a_seq;
            axi_pkg::axi_slave_sequence#(64,64,8,3,1) axi_slave_gmem_b_seq;
            axi_pkg::axi_slave_sequence#(64,64,8,3,1) axi_slave_gmem_c_seq;
            ...
            if (!uvm_config_db#(matmul_full_contiguous_reference_model)::get(p_sequencer,"", "refm", refm))
                ...
                begin
                    fork
                        begin //axi slave sequence. loop delays
                            `uvm_create_on(axi_slave_gmem_a_seq, p_sequencer.gmem_a_sqr);
                            axi_slave_gmem_a_seq.misc_if = refm.misc_if;
                            axi_slave_gmem_a_seq.ap_done    = refm.ap_done_for_nexttrans   ;
                            axi_slave_gmem_a_seq.ap_ready   = refm.ap_ready_for_nexttrans  ;
                            axi_slave_gmem_a_seq.finish     = refm.finish ;
                            axi_slave_gmem_a_seq.isusr_delay = axi_pkg::NO_DELAY;
                            `uvm_send(axi_slave_gmem_a_seq);
                        end
                        begin //axi slave sequence. loop delays
                            `uvm_create_on(axi_slave_gmem_b_seq, p_sequencer.gmem_b_sqr);
                            axi_slave_gmem_b_seq.misc_if = refm.misc_if;
                            axi_slave_gmem_b_seq.ap_done    = refm.ap_done_for_nexttrans   ;
                            axi_slave_gmem_b_seq.ap_ready   = refm.ap_ready_for_nexttrans  ;
                            axi_slave_gmem_b_seq.finish     = refm.finish ;
                            axi_slave_gmem_b_seq.isusr_delay = axi_pkg::NO_DELAY;
                            `uvm_send(axi_slave_gmem_b_seq);
                        end
                        begin //axi slave sequence. loop delays
                            `uvm_create_on(axi_slave_gmem_c_seq, p_sequencer.gmem_c_sqr);
                            axi_slave_gmem_c_seq.misc_if = refm.misc_if;
                            axi_slave_gmem_c_seq.ap_done    = refm.ap_done_for_nexttrans   ;
                            axi_slave_gmem_c_seq.ap_ready   = refm.ap_ready_for_nexttrans  ;
                            axi_slave_gmem_c_seq.finish     = refm.finish ;
                            axi_slave_gmem_c_seq.isusr_delay = axi_pkg::NO_DELAY;
                            `uvm_send(axi_slave_gmem_c_seq);
                        end
                        begin
                            ...
                            for(int i=0; i<1; i++) begin
                                ...
                                for(int j=0; j < (64+32-1)/32; j++) begin
                                    data64bit_a.push_back( ((refm.mem_blk_pages_gmem_a.maxi_bundlevar_offset["a"]+refm.mem_blk_pages_gmem_a.page_ofst[refm.mem_blk_pages_gmem_a.rd_page_idx])>>(j*32)) & (2**32-1) );
                                end
                                ...
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                for(int j=0; j < (64+32-1)/32; j++) begin
                                    data64bit_b.push_back( ((refm.mem_blk_pages_gmem_b.maxi_bundlevar_offset["b"]+refm.mem_blk_pages_gmem_b.page_ofst[refm.mem_blk_pages_gmem_b.rd_page_idx])>>(j*32)) & (2**32-1) );
                                end
                                ...
                                axi_master_wr_control_seq.StableAxiliteNoUpdate=0;
                                for(int j=0; j < (64+32-1)/32; j++) begin
                                    data64bit_c.push_back( ((refm.mem_blk_pages_gmem_c.maxi_bundlevar_offset["c"]+refm.mem_blk_pages_gmem_c.page_ofst[refm.mem_blk_pages_gmem_c.rd_page_idx])>>(j*32)) & (2**32-1) );
                                end
            ...
            p_sequencer.gmem_a_sqr.stop_sequences();
            p_sequencer.gmem_b_sqr.stop_sequences();
            p_sequencer.gmem_c_sqr.stop_sequences();
